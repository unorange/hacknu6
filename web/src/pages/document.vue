...
const document = route.query.document;

const { data: doesntExist, error } = await useFetch(
  `${BASE_URL}/check-document/${document}`
);
...
const { active: loadingAuthorization, run: authorize } = useAsync(
  api.authorize
);
const { active: loadingLocations, run: loadLocations } = useAsync(
  api.getLocations
);
...
const { active: calculatingPrice, run: loadCalculation } = useAsync(
  api.calculatePrice
);
...
const next = async () => {
  ...
  if (currentStep.value === 3) {
    const [data, error] = await makeDelivery(
      ...
      acceptedBy.value === "me"
        ? Number(iin.value)
        : Number(representativeIIN.value)
    );
...
const autocompleteOptions = computed(() => {
  ...
  return flatCities.value
    .filter((city) =>
      city.toLowerCase().includes(selectedCity.value.toLowerCase())
    )
...
watch(isAvailableCity, async (newValue, oldValue) => {
  ...
  const [deliveryServicesData, deliveryServicesError] =
    await loadDeliveryServices(region.id);
...
<template>
  <template v-if="!doesntExist || error">
    <div class="p-2">
      ...
      <template v-if="currentStep === 1">
        <div>
          ...
          <div class="mb-2" v-if="validIIN">
            <label class="text-base mb-1 text-zinc-500">
              Одноразовый код
            </label>
      ...
      <template v-else="currentStep === 3">
        <template v-if="calculated">
          <h1 class="text-xl mb-2">
            Стоимость доставки: {{ calculated.price }}₸
          </h1>
        </template>
      </template>
      <n-button
  ...
  <n-modal v-model:show="showSuccessPaymentModal">
    <n-alert title="Заказ оформлен" type="success">
      Оплата прошла успешно
    </n-alert>
  </n-modal>
  <n-modal v-model:show="showErrorPaymentModal">
    <n-alert title="Прозошла ошибка" type="error">
